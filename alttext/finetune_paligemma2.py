...
JSONL_PATH = os.path.join(DATASETS_DIR, "hci_classified.jsonl")
...
# Parse the jsonl file for specific class finetuning
parser = argparse.ArgumentParser()
parser.add_argument("--figure-class", type=str, default=None,
                    help="Filter to a specific figure_class (e.g. 'diagram', 'meta figure')")
cli_args = parser.parse_args()

# Load Dataset
ds = load_dataset("json", data_files=JSONL_PATH, split="train")

if cli_args.figure_class:
    before = len(ds)
    ds = ds.filter(lambda x: x.get("figure_class") == cli_args.figure_class)
    print(f"Filtered to '{cli_args.figure_class}': {len(ds)}/{before} records")
...
# Train
# -----------------------
class_slug = cli_args.figure_class.replace(" ", "_") if cli_args.figure_class else "all"
output_dir = f"paligemma-alttext-lora/{class_slug}"
...
# For diagrams:
# python3 finetune_paligemma2.py --figure-class "diagram"

# For meta figures:
# python3 finetune_paligemma2.py --figure-class "meta figure"